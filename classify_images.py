...
def classify_images(images_dir, results_dic, model):
    ...
    for key in results_dic:
        ...
        # Set lower case and strip the leading and trailing whitespace characters
        classifier_labels = classifier_labels.lower().strip()
        
        # Define pet image label and place from results_dic dictionary
        pet_labels = results_dic[key][0]
        
        # Compare pet image label with classifier label
        results_dic[key] = [ pet_labels, classifier_labels ]
        
        # If pet image label is found within classifier label then they match
        if pet_labels in classifier_labels:
            results_dic[key].append(1)
            
        # If pet image label is not found within classifier label then they don't match
        else:
